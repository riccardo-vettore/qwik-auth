...
import {globalAction$, type RequestEvent, type RequestEventCommon, routeLoader$, z, zod$,} from '@builder.io/qwik-city';
...
export function serverAuthQrl(authOptions: QRL<(ev: RequestEventCommon) => QwikAuthConfig>) {
    ...
    const getProviders = routeLoader$(
        async (req) => {
            const auth = await authOptions(req);
            const url = new URL('/api/auth/providers', req.url);
            const response = await Auth(new Request(url, {headers: req.headers}), auth);
